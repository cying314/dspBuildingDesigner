...
export function isAssembler(id) {
  return assemblerBuildIds.has(id);
}
...
/** 是否 可悬空建造 */
export function isHanging(id) {
  return hangingBuildIds.has(id);
}
...
export function isStackable(id) {
  return stackableBuildIds.has(id);
}
...
/** 是否 带分拣器插槽的建筑 */
export function isInserterSlotBuild(id) {
  return Object.prototype.hasOwnProperty.call(inserterSlotBuildInfos, id);
}

/** 获取 带分拣器插槽的建筑 翻转信息 */
export function getInserterSlotBuildInfo(id) {
  return inserterSlotBuildInfos[id];
}

/** 获取 带分拣器插槽的建筑 的对称方向 */
export function getInserterSlotBuildAxis(id) {
  return getInserterSlotBuildInfo(id)?.axis;
}
...
/** 是否 带传送带插槽的建筑 */
export function isBeltSlotBuild(id) {
  return Object.prototype.hasOwnProperty.call(beltSlotBuildInfos, id);
}
...
/** 获取 传送带插槽的建筑 的对称方向 */
export function getBeltSlotBuildAxis(id, modelIndex) {
  return getBeltSlotBuildInfo(id, modelIndex)?.axis;
}
...
/** 是否 炮台 */
export function isTurret(id) {
  return turretBuildIds.has(id);
}

/** 根据物品id获取物品图标 */
export function getItemIcon(itemId) {
  let icon = itemsMap.get(itemId)?.icon;
  if (!icon) return null;
  try {
...
export function getItemImage(itemId) {
  ...
  let img = itemImageMap.get(itemId);
  if (img) return img;
  try {
    img = require("@/assets/images/items/" + itemId + ".png");
    itemImageMap.set(itemId, img);
    return img;
  } catch (e) {
    return null;
  }
...
export function getSignalImage(signalId) {
  ...
  if (img) return img;
  try {
    img = require("@/assets/images/signals/" + signalId + ".png");
    signalImageMap.set(signalId, img);
    return img;
  } catch (e) {
    return null;
  }
